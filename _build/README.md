...
| 4b | `04b_normalize.ps1` `[-Only <pat>] [-Fresh]` | `.mp3` du `tree` → `audio.target_lufs` / `audio.target_tp` + `normalize_report.csv` (delta before/after, fichiers traités uniquement ; « avant » lu de `ffmpeg-normalize --print-stats`, pas de passe ffmpeg en plus). **Idempotent** : normalise depuis la copie pristine `work\raw_audio` ; chaque générateur (02/03/04) invalide ses propres backups. Run complet ≈ 10 min (les 2 passes de `ffmpeg-normalize` dominent) — itérer avec `-Only` |
...
4. `01_inventory.ps1` → `02_merge.ps1` → étapes actives → `04b_normalize.ps1` → `05_run_spg.ps1` → `05b_check_story.py`.
