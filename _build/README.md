...
| 7 | `07_icons.py` (`uv … --with svglib --with reportlab --with lxml`) | si `icons.mode="emoji"` : pictogrammes OpenMoji (blanc sur near-black) pour les thèmes (`categories[].icon`) et les histoires (`story_icons`, clé = clé d'histoire). `first_menu_audio_only` supprime `<menu>/0-item.png`. Cache PNG namespacé par `icons.set`. Écrit `work/icons_report.json` (couverture, lu par 05) |
