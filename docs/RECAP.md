...
## #3 — audit mécanique effectué (issue laissée ouverte pour l'éditorial)

Vérifié propre : références script/chemin/flag/clé, défauts cités == `DEFAULTS`,
schéma complet (0 clé fantôme après #22b9149), miroir PS/Python en phase, énums de
mode == code, ordre pipeline, `LICENSE` présent, attribution OpenMoji, `.gitignore`.
Reste éditorial (arbitrage auteur) : relecture des docstrings, narration du cas
générique, bloc `tools` résiduel dans `project.example.json`. Détail :
commentaire sur l'issue #3.

## Restent ouvertes — arbitrage auteur / matériel Lunii / gros chantiers

- **Éditorial / audio** : #3 (points éditoriaux ci-dessus), #6 (audio cible + tests A/B sur la conteuse)
- **Investigation avant code** : #14 (cache whisper — bloqué par #16), #9 (harness de test e2e)
- **Décision produit** : #16 (titres en TTS), #17 (meilleure détection titre), #19 (scoring pochettes), #21 (se passer de SPG), #28 (pré-gain par chapitre), #5 (Spytify), #12 (orchestrateur — l'issue penche statu quo)
- **Gros refactors** : #10 (Linux/macOS), #11 (tout-Python), #24 (disposition du dépôt)
