...
| #3 (partiel) | Revue de la documentation | Clé fantôme `titles.strip_key_word` supprimée : présente dans `DEFAULTS` (PS + Python) et posée dans `examples/timote.json` depuis le refactor #1, lue par aucun script. Le reste de #3 (audit complet doc/défauts/altitude) reste ouvert. | _(ce commit)_ |
