...
| #26 | Mesure de loudness dupliquée dans 01 / 04b / 04d | Fonction unique `Measure-Loudness` dans `_config.ps1` (une passe ffmpeg `ebur128`, parse `Summary:`). Les trois `Measure-LU` / bloc inline locaux supprimés. Sorties `inventory.csv` / `stories.json` byte-identiques avant/après ; 04d re-vérifié. | _(ce commit)_ |
